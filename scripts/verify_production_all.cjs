...
async function runVerification() {
  ...
  const report = {
    ...
    mapStability: { mapLoaded: false, canPan: false, canZoom: false, recenterWorks: false },
  ...
  try {
    ...
    const desktopUiChecks = await pageDesktop.evaluate(() => {
      ...
      return { hasOfficialAlert, hasLocalThreats, hasUpdatedBadge, diagnosticsCollapsed };
    });

    report.simplifiedUI = desktopUiChecks;
    ...
    // Verify Leaflet Map loaded & check zoom
